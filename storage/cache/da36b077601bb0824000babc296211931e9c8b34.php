<!--! ================================================================ !-->
<!--! [Start] Navigation Manu !-->
<!--! ================================================================ !-->
<nav class="nxl-navigation">
    <div class="navbar-wrapper">
        <div class="m-header">
            <a href="/" class="b-brand">
                <!-- ========   change your logo hear   ============ -->
                <img src="https://thanhbui.click/wp-content/uploads/2025/09/tbs-removebg-preview.png"
                    style="width: 100px;" alt="" class="logo logo-lg" />
                <img src="https://thanhbui.click/wp-content/uploads/2025/09/tbs-removebg-preview.png"
                    style="width: 50px;" alt="" class="logo logo-sm" />
            </a>
        </div>
        <div class="navbar-content">
            <ul class="nxl-navbar">
                <li class="nxl-item nxl-caption">
                    <label>Điều hướng</label>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <a href="/dashboard" class="nxl-link">
                        <span class="nxl-micon"><i class="feather-airplay"></i></span>
                        <span class="nxl-mtext">Bảng điều khiển</span>
                    </a>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <div class="nxl-link" style="cursor: pointer;" onclick="if(event.target.closest('.nxl-arrow')) { return; } else { window.location.href='/product'; event.stopPropagation(); }">

                        <span class="nxl-micon">
                            <i class="feather-shopping-cart"></i>
                        </span>

                        <span class="nxl-mtext">Sản phẩm</span>

                        <span class="nxl-arrow">
                            <i class="feather-chevron-right"></i>
                        </span>

                    </div>

                    <ul class="nxl-submenu" style="display: none;">
                        <li class="nxl-item">
                            <a class="nxl-link" href="/color">
                                <span class="nxl-micon">
                                    <i class="feather-droplet"></i>
                                </span>
                                Màu sắc
                            </a>
                        </li>

                        <li class="nxl-item">
                            <a class="nxl-link" href="/rom">
                                <span class="nxl-micon">
                                    <i class="feather-hard-drive"></i>
                                </span>
                                Bộ nhớ
                            </a>
                        </li>
                    </ul>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <a href="/category" class="nxl-link">
                        <span class="nxl-micon"><i class="feather-layers"></i></span>
                        <span class="nxl-mtext">Danh mục</span>
                    </a>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <a href="/trademark" class="nxl-link">
                        <span class="nxl-micon"><i class="feather-award"></i></span>
                        <span class="nxl-mtext">Thương hiệu</span>
                    </a>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <a href="/voucher" class="nxl-link">
                        <span class="nxl-micon"><i class="feather-gift"></i></span>
                        <span class="nxl-mtext">Voucher</span>
                    </a>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <a href="/contact/admin" class="nxl-link">
                        <span class="nxl-micon"><i class="feather-message-circle"></i></span>
                        <span class="nxl-mtext">Liên hệ</span>
                    </a>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <a href="/order/index" class="nxl-link">
                        <span class="nxl-micon"><i class="feather-clipboard"></i></span>
                        <span class="nxl-mtext">Đơn hàng</span>
                    </a>
                </li>
                <li class="nxl-item nxl-hasmenu">
                    <a href="/user" class="nxl-link">
                        <span class="nxl-micon"><i class="feather-users"></i></span>
                        <span class="nxl-mtext">Người dùng</span>
                    </a>
                </li>
            </ul>
        </div>
    </div>
</nav>
<!--! ================================================================ !-->
<!--! [End]  Navigation Manu !-->
<!--! ================================================================ !--><?php /**PATH C:\xampp\htdocs\lab2-30-1-26\PHP2\app\views/layouts/includes/slidebar_admin.blade.php ENDPATH**/ ?>