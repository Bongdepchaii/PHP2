  ...
  <div class="card shadow-sm border-0 rounded-3 mb-3">
    ...
    <div class="card-body pt-0">
      ...
      <div class="d-flex justify-content-between mb-2">
        ...
        <span class="fw-semibold" id="sidebar-count">{{ array_sum(array_column($cart, 'quantity')) }} cái</span>
      </div>
      <div class="d-flex justify-content-between mb-3">
        <span class="text-muted">Tạm tính:</span>
        <span class="fw-bold" id="sidebar-subtotal">{{ number_format($subtotal, 0, ',', '.') }}đ</span>
      </div>
      ...
      <div class="alert alert-success py-2 px-3 d-flex justify-content-between align-items-center mb-3">
        <div>
          ...
          <strong>{{ $voucherInfo['id'] }}</strong>
          <br><small>Giảm {{ $voucherInfo['value'] }}% — Tiết kiệm {{ number_format($discount, 0, ',', '.') }}đ</small>
        </div>
      ...
      <div class="d-flex justify-content-between mb-2 text-danger">
        <span>Giảm ({{ $voucherInfo['value'] }}%):</span>
        <span>- <span id="sidebar-discount">{{ number_format($discount, 0, ',', '.') }}</span>đ</span>
      </div>
      @endif

      {{-- Nút chọn / nhập mã voucher --}}
      ...
      <div class="d-flex justify-content-between align-items-center mt-3">
        ...
        <span class="h5 mb-0 text-primary fw-bold" id="sidebar-total">{{ number_format($total, 0, ',', '.') }}đ</span>
      </div>
      ...
      <div class="modal-body">
        ...
        <div class="d-flex flex-column gap-2">
          ...
          <div class="border rounded-3 p-3 d-flex justify-content-between align-items-center">
            <div>
              <span class="fw-bold text-success d-block">{{ $v['id'] }}</span>
              <small class="text-muted">{{ $v['name'] }} — Giảm {{ number_format($v['value'], 0, ',', '.') }}%</small>
              <small class="text-muted d-block">HSD: {{ date('d/m/Y', strtotime($v['end_date'])) }} | Còn {{ $v['quanity'] }} lượt</small>
            </div>
            <form action="/cart/applyVoucher" method="POST">
              <input type="hidden" name="voucher_code" value="{{ $v['id'] }}">
    ...
    <form action="/cart/checkout" method="POST">
      <div class="modal-content border-0 shadow">
        ...
        <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <div class="d-flex flex-column gap-2" id="savedAddressList">
              ...
              <div class="border rounded-3 p-2 d-flex align-items-center gap-2 saved-addr-item"
                   ...
                   data-receiver="{{ $addr['receiver'] }}"
                   data-phone="{{ $addr['phone'] }}"
                   data-address="{{ $addr['address'] }}">
                <input type="radio" name="addr_choice" value="{{ $addr['id'] }}"
                       id="addr_{{ $addr['id'] }}"
                       {{ $addr['is_default'] ? 'checked' : '' }}>
                <label for="addr_{{ $addr['id'] }}" style="cursor:pointer" class="mb-0">
          ...
          {{-- Form điền tay (hiển thị nếu không có địa chỉ hoặc chọn "nhập khác") --}}
          <div id="manualForm" {{ (isset($addresses) && count($addresses) > 0) ? 'style=display:none' : '' }}>
            <div class="row g-2">
              <div class="col-md-6">
          ...
          <div class="mt-3 p-3 bg-light rounded-3">
            ...
            <div class="d-flex justify-content-between text-danger">
              <span>Giảm giá ({{ $voucherInfo['id'] ?? '' }}):</span>
